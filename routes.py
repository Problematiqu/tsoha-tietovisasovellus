...
def quiz():
    ...
    for question in questions:
        ...
        form.question[i].choices = [
            (questions[question][0].id, questions[question][0].option),
            (questions[question][1].id, questions[question][1].option),
            (questions[question][2].id, questions[question][2].option),
            (questions[question][3].id, questions[question][3].option)]
        i += 1
...
def scoreboard():
    scores = sql_commands.get_scoreboard()
...
def add():
    ...
    if request.method == "GET":
        ...
        if form.validate_on_submit():
            sql_commands.add_question(form.question.data, int(form.topic.data),
                                      form.correct.data, form.incorrect.data)
