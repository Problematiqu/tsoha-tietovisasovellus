...
class AddForm(FlaskForm):
    question = StringField("Kysymys:", validators=[
        InputRequired(message="Kysymys ei saa olla tyhjä")])
    topics = sql_commands.get_topics()
    topics = [(topic[0], topic[1]) for topic in topics]
    topic = SelectField("Aihealue", validators=[
        InputRequired(message="Aihealue ei saa olla tyhjä")],
                        choices=topics)
    correct = StringField("Oikea vastaus:", validators=[
        InputRequired(message="Vastaus ei saa olla tyhjä")])
    incorrect = FieldList(StringField("Väärä vastaus:", validators=[
        InputRequired(message="Vastaus ei saa olla tyhjä")]),
                          min_entries=3)
    submit = SubmitField("Lähetä")

class QuizForm(FlaskForm):
    question = FieldList(RadioField(validators=[
        InputRequired(message="Valitse yksi")],
                                    coerce=int,
                                    choices=[]),
                         min_entries=10)
    submit = SubmitField("Lähetä")
